...
const agregarCarrito = (guitarra) => {
  const existsCarrito = carrito.value.findIndex(item => item.id === guitarra.id)
  if (existsCarrito >= 0) {
    if (carrito.value[existsCarrito].cantidad >= 5) return
    carrito.value[existsCarrito].cantidad++
  } else {
    guitarra.cantidad = 1;
